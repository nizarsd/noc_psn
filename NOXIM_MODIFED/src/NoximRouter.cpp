...
void NoximRouter::rxProcess()
{
	...
	for (int i = 0; i < DIRECTIONS + 1; i++) {
		...
		stats.power.Incoming();
	       ...
	       // Cycle	router_id	process_id	process_power  <Nizar>
               if (NoximGlobalParams::verbose_mode > VERBOSE_OFF) {
               cout << sc_time_stamp().to_double() / 1000
				<<","<< local_id
                                <<","<< PROCESS_INCOMING << endl;
				//<<","<< stats.power.getPwrIncoming()<<endl;
               }   
	    }
    ...
    cout << sc_time_stamp().to_double() / 1000
				<<","<< local_id
                                <<","<< PROCESS_STANDBY << endl;
				//<<","<< stats.power.getPwrStandBy()<<endl;
    }
}
...
void NoximRouter::txProcess()
{
	...
	for (int j = 0; j < DIRECTIONS + 1; j++) {
		...
		if (flit.flit_type == FLIT_TYPE_HEAD) {
			...
			if (NoximGlobalParams::verbose_mode > VERBOSE_OFF) {
			   // AND CHANGED THIS !!!!!! 
		           // RESERVING OUPUT (INJECT FLIT TO BUFFER)
	...
	for (int i = 0; i < DIRECTIONS + 1; i++) {
		...
		if (o != NOT_RESERVED) {
			...
			stats.power.Forward();
                        if (NoximGlobalParams::verbose_mode > VERBOSE_OFF) {
                           // AND FINALY CHANGED THIS !!!!!! 
			   // ROUTER TRANSMITTING FLITS (ACTUAL BUS LOAD)
			   ...
			   /* cout << sc_time_stamp().to_double() / 1000
				...
				<< flit << endl; */
                          ...
                          // Wrote this to print the src dir_came_from dir_going_to
                          // if(o!=4){   // I may need to use this to extract only global communication
                          
                          // Cycle	router_id	process_id	process_power outputchannel (why!) <Nizar>
                               cout << sc_time_stamp().to_double() / 1000
				<< "," << local_id
                                << "," << PROCESS_FORWARD
				//<< "," << stats.power.getPwrForward()
				<< "," << o << endl; 
...
int NoximRouter::route(const NoximRouteData & route_data)
{
	...
	if (NoximGlobalParams::verbose_mode > VERBOSE_OFF) {
	    // Cycle	router_id	process_id	process_power  <Nizar>
                               cout << sc_time_stamp().to_double() / 1000
				<<","<< local_id
                                <<","<< PROCESS_ROUTING << endl;
				//<<","<< stats.power.getPwrRouting()<<endl;
	}
				   ...
				   const NoximRouteData & route_data)
{
    ...
    // Cycle	router_id	process_id	process_power  <Nizar>
                               cout << sc_time_stamp().to_double() / 1000
				<<","<< local_id
                                <<","<< PROCESS_SELECTION << endl;
				//<<","<< stats.power.getPwrSelection()<<endl;
    }
