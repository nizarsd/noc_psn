...
The tool is written in MATLAB and can run on Octave too. It is highly recommended to use Linux to run this tool It requires installing modified version on NOXIM (the traffic simulator) which will provide the traffic output in the format needed by this tool to compute the router power. The sources for this modified noxim are provided under "MODIFIED_NOXM" folder
